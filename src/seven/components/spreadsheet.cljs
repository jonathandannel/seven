    ...
    ; A-Z titles
       [:thead
        [:tr
         ...
         (map (fn [x] [:th.spreadsheet-title-letter {:key (str "title-" x)
                                                     :class
                                                     (if (= x (first active))
                                                       "spreadsheet-active-letter")}  x]) a-to-z)]]
       [:tbody
        (map
         (fn [number]
           [:tr {:key (str "row-" "number-" number)}
         ; Row number title on side
            [:th.spreadsheet-title-number {:key (str "title-" number)
                                           :class
